...
SAVE
  ...
  real(field_r) :: ysizelocal
  real(field_r) :: dsigma2dx_sgs=0, dsigma2dy_sgs=0, dsigma2dz_sgs=0, dsigma2dt_sgs=0,sigma2_new = 0
  real(field_r) :: fce
  real(field_r),allocatable :: fsm(:)
  integer :: np

  TYPE :: particle_record
    real :: unique, tstart
    integer :: partstep
    real(field_r) :: x,x_prev, xstart, ures, usgs, usgs_prev
    real(field_r) :: y,y_prev, ystart, vres, vsgs, vsgs_prev
    real(field_r) :: z,z_prev, zstart, wres, wsgs, wsgs_prev
    real(field_r) ::sigma2_sgs
...
!*****************************************************************************
  subroutine initparticles
    use modmpi,   only : myid, mpierr, comm3d, nprocs, d_mpi_bcast
  ...
  ! LOCAL
    integer :: n, ierr
    real(field_r) :: tstart, xstart, ystart, zstart
  ...
  !read namelist and broadcast it to all processors
    ...
    call D_MPI_BCAST(lpartic     ,1,0,comm3d,mpierr)
    call D_MPI_BCAST(timeav      ,1,0,comm3d,mpierr)
    call D_MPI_BCAST(dtav        ,1,0,comm3d,mpierr)
    call D_MPI_BCAST(timedump    ,1,0,comm3d,mpierr)
    call D_MPI_BCAST(npartdump   ,1,0,comm3d,mpierr)
    call D_MPI_BCAST(intmeth     ,1,0,comm3d,mpierr)
    call D_MPI_BCAST(lpartsgs    ,1,0,comm3d,mpierr)
    call D_MPI_BCAST(lstat       ,1,0,comm3d,mpierr)
    call D_MPI_BCAST(ldump       ,1,0,comm3d,mpierr)
...
!*****************************************************************************
  function velocity_ures(x, y, z)
    ...
    implicit none
    real(field_r), intent(in) ::  x, y, z
    integer :: xbottom, ybottom, zbottom
    real(field_r) :: velocity_ures, deltax, deltay, deltaz
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  function velocity_vres(x, y, z)
    ...
    !local::
    integer :: xbottom, ybottom, zbottom
    real(field_r) ::  velocity_vres, deltax, deltay, deltaz
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  function velocity_wres(x, y, z)
    ...
    !local::
    integer :: xbottom, ybottom, zbottom
    real(field_r) :: velocity_wres, deltax, deltay, deltaz
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine statistics
    ...
    use modmpi,    only : comm3d,mpierr,mpi_sum,myid,d_mpi_allreduce
    implicit none
    ...
    real(field_r),allocatable, dimension(:) :: uresavl  ,uresav,&
                                       vresavl  ,vresav,&
                                       wresavl  ,wresav,&
                                       u2resavl ,u2resav,&
                                       v2resavl ,v2resav,&
    ...
    if (timee>=tnext) then
      ...
      call D_MPI_ALLREDUCE(nrparticlesavl,nrparticlesav, k1, MPI_SUM, comm3d,mpierr)
      call D_MPI_ALLREDUCE(uresavl,uresav, k1, MPI_SUM, comm3d,mpierr)
      call D_MPI_ALLREDUCE(vresavl,vresav, k1, MPI_SUM, comm3d,mpierr)
      call D_MPI_ALLREDUCE(wresavl,wresav, k1, MPI_SUM, comm3d,mpierr)
      call D_MPI_ALLREDUCE(u2resavl,u2resav, k1, MPI_SUM, comm3d,mpierr)
      call D_MPI_ALLREDUCE(v2resavl,v2resav, k1, MPI_SUM, comm3d,mpierr)
      call D_MPI_ALLREDUCE(w2resavl,w2resav, k1, MPI_SUM, comm3d,mpierr)
      call D_MPI_ALLREDUCE(usgsavl,usgsav, k1, MPI_SUM, comm3d,mpierr)
      call D_MPI_ALLREDUCE(vsgsavl,vsgsav, k1, MPI_SUM, comm3d,mpierr)
      call D_MPI_ALLREDUCE(wsgsavl,wsgsav, k1, MPI_SUM, comm3d,mpierr)
      call D_MPI_ALLREDUCE(u2sgsavl,u2sgsav, k1, MPI_SUM, comm3d,mpierr)
      call D_MPI_ALLREDUCE(v2sgsavl,v2sgsav, k1, MPI_SUM, comm3d,mpierr)
      call D_MPI_ALLREDUCE(w2sgsavl,w2sgsav, k1, MPI_SUM, comm3d,mpierr)
      call D_MPI_ALLREDUCE(tkesgsavl,tkesgsav, k1, MPI_SUM, comm3d,mpierr)
 ...
 !*****************************************************************************
  subroutine writeparticles
    ...
    ! LOCAL
    real(field_r), allocatable,dimension (:,:) :: partdata
    integer(KIND=selected_int_kind(11)), allocatable, dimension (:) :: partids
    integer :: n,m,ndata
    type (particle_record), pointer:: particle
    real(field_r) :: thlpart, qtpart, qlpart, thvpart, exnpart,prespart,tl,es,qs,qsl,b1
    ...
    if (timee>=tnextdump) then
      ...
      do while( associated(particle) )
        ...
        qlpart  = dim(qtpart-qs,0.0_field_r)
...
!fce
  ...
  call D_MPI_ALLREDUCE(umslabl,  umslab, k1, MPI_SUM, comm3d,mpierr)
  call D_MPI_ALLREDUCE(vmslabl,  vmslab, k1, MPI_SUM, comm3d,mpierr)
  call D_MPI_ALLREDUCE(um2slabl,um2slab, k1, MPI_SUM, comm3d,mpierr)
  call D_MPI_ALLREDUCE(vm2slabl,vm2slab, k1, MPI_SUM, comm3d,mpierr)
  call D_MPI_ALLREDUCE(wm2slabl,wm2slab, k1, MPI_SUM, comm3d,mpierr)

  call D_MPI_ALLREDUCE(tkesgsmslabl,tkesgsmslab,k1,MPI_SUM, comm3d,mpierr)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine sgshelpvar(particle)
    ...
    real(field_r) :: c0 = 6
      real(field_r) :: fs, ce, sgsdthvdz
      integer :: xbottom, ybottom, zbottom
      real(field_r) :: deltax,deltay,deltaz
    TYPE (particle_record), POINTER:: particle
    ...
    dsigma2dx_sgs = 2./(3*dx)*(sgstke(real(xbottom+1.5,kind=field_r),particle%y,particle%z   ) - &
                  sgstke(real(xbottom+0.5,kind=field_r)  ,particle%y,particle%z))
    dsigma2dy_sgs = 2./(3*dy)*(sgstke(particle%x, real(ybottom+1.5,kind=field_r),particle%z) - &
                  sgstke(particle%x, real(ybottom+0.5,kind=field_r)  ,particle%z))
    dsigma2dz_sgs = 2./(3*dzh(zbottom+1))* &
                 (sgstke(particle%x, particle%y, real(zbottom+1.5,kind=field_r) ) - &
                  sgstke(particle%x, particle%y, real(zbottom+0.5,kind=field_r)))

    dsigma2dt_sgs = (log(sigma2_new)-log(particle%sigma2_sgs))/rdt

    dsigma2dt_sgs = sign(1.0_field_r,dsigma2dt_sgs)*min(abs(dsigma2dt_sgs),1.0_field_r/rdt)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  function sgstke(x, y, z)
    ...
    real(field_r) :: sgstke, deltax, deltay, deltaz
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  function gauss1(idum)
    ...
    real(field_r) :: gauss1, fac, gset, rsq, v1, v2
    save iset, gset
    data iset /0/
    rsq = 0.
    v1 = 0.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  function ran1(idum)
    ...
    real(field_r) :: ran1, am, eps1, rnmx
    ...
    ia = 16807
    im = 2147483647
    am = 1. / real(im,kind=field_r)
    iq = 127773
    ir = 2836
    ndiv = 1 +  (im-1)/real(ntab,kind=field_r)
    eps1 = 1.2E-7
    rnmx = 1. - eps1
  ...
  subroutine checkbound(particle)
    ...
    particle%x      = modulo(particle%x-2,real(imax,kind=field_r))+2
    particle%x_prev = modulo(particle%x_prev-2,real(imax,kind=field_r))+2
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  function scalintp(x, y, z,field,field0)
    ...
    real(field_r), intent(in) :: x, y, z
    real(field_r), dimension(:,:,:), intent(in) :: field
    real(field_r), intent(in) :: field0

    !local::
    integer :: xbottom, ybottom, zbottom

    real(field_r) :: scalintp, deltax, deltay, deltaz
...
!*****************************************************************************
  ...
  subroutine partcommunicate
    ...
    use modmpi,    only : comm3d,mpierr,nbrnorth,nbrsouth,mpi_status,d_mpi_sendrecv
    implicit none
    ...
    real(field_r), allocatable, dimension(:) :: buffsend, buffrecv
    ...
    call D_MPI_SENDRECV(nrtonorth,1,nbrnorth,4, &
              nrfrsouth,1,nbrsouth,4, &
              comm3d, status, mpierr)

    call D_MPI_SENDRECV(nrtosouth,1,nbrsouth,5, &
              nrfrnorth,1,nbrnorth,5, &
              comm3d, status, mpierr)
    ...
    call D_MPI_SENDRECV(buffsend,nrpartvar*nrtonorth,nbrnorth,6, &
              buffrecv,nrpartvar*nrfrsouth,nbrsouth,6, &
              comm3d, status, mpierr)
    ...
    call D_MPI_SENDRECV(buffsend,nrpartvar*nrtosouth,nbrsouth,7, &
              buffrecv,nrpartvar*nrfrnorth,nbrnorth,7, &
              comm3d, status, mpierr)
    ii = 0
    do n = 1,nrfrnorth
      particle => head
...
!*****************************************************************************
  subroutine partfillbuffer(particle, buffer, n,send)
    ...
    integer,intent(in) :: n
    real(field_r),dimension(n+1:n+nrpartvar) :: buffer
    TYPE (particle_record), POINTER:: particle
